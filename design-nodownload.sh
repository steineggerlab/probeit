...
if [[ "$PARAMCNT" -lt 3 ]]; then
    echo "Usage: design-nodownload.sh -i input.fas -l prob_len -o output_folder"

    echo "REQUIRED"
    echo " -i|--input fasta input"
...
# setcover the k-mers  
mkdir setcover
setcover mapping/uniq.genmap.csv $COVERAGE "$PROBLEN1" 1.0 > "setcover/result"
...
# minimize 20-mers
setcover mapping_20/uniq.genmap.csv 1 1 1.0 > "setcover_20/result"
